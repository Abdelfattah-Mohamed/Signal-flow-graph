...
public class input_screen implements Initializable{
	...
	public void add_branch(ActionEvent event) {
		...
		if(!mat1.matches() || !mat2.matches() || !mat3.matches()) {
			...
			//7war el ip wl op nodes da ha ?
			
			
			/**if(fromNode == num_of_nodes || toNode==1) {
				Alert alert = new Alert(AlertType.ERROR, "the source node (1) can not have an ingoing branch "
						+ "and the sink node (" +num_of_nodes +") can not have an outgoing branch ", ButtonType.OK);
			}*/
